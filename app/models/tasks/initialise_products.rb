  ...
  class InitialiseProducts < Task
    ...
    def execute
      External::Product.left_joins(:category).includes(:translations).managed.find_each do |product|
        Product.find_or_create_by!(external_product_id: product.id, data_source: :import).tap do |p|
          ...
          product.translations.each do |translation|
            ProductTranslation.find_or_create_by!(
              product: p,
              external_product_translation_id: translation.id,
              data_source: :import
            ) do |t|
              t.update!(
