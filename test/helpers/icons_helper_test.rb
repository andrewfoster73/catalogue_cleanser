...
class IconsHelperTest < ActionView::TestCase
  test 'icon with default settings' do
    svg_html = <<~HTML.squish
      <svg class="text-gray-400 group-hover:text-gray-300 mr-2 flex-shrink-0 h-6 w-6" 
  ...
  test 'icon that is active' do
    svg_html = <<~HTML.squish
      <svg class="text-yellow-300 mr-2 flex-shrink-0 h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" 
  ...
  test 'icon with a different size' do
    svg_html = <<~HTML.squish
      <svg class="text-red-400 group-hover:text-red-300 mr-2 flex-shrink-0 h-9 w-9" xmlns="http://www.w3.org/2000/svg" 
    ...
    assert_dom_equal(svg_html, icon(name: :trash, colour: :red, options: { size: 9 }))
  end
end
