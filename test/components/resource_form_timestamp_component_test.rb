...
class ResourceForm::TimestampComponentTest < ViewComponent::TestCase
  ...
  test 'editable mode' do
    render_inline(ResourceForm::FieldComponent.new) do |component|
      component.with_attribute_timestamp(attribute: :created_at, label: 'Created At', resource: @resource,
                                         readonly: false
      )
    end

    assert_selector('#item_sell_pack_created_at--label')
    assert_selector('label[for="item_sell_pack_created_at"]', text: 'Created At')
    assert_selector(
      'p#item_sell_pack_created_at',
  ...
  test 'with help text' do
    ...
    assert_selector('#item_sell_pack_created_at--help', text: 'This is a help message')
  end
end
